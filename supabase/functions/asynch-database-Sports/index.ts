...
async function runTwitterXScraper(): Promise<any[]> {
  ...
  const params = {
    ...
    "lang": "en",
    "list": "1922535797959356681",  // Sports list ID
    "maxItems": 200,
    "queryType": "Top",
    "within_time": "60d"
  };
...
async function processTweets(tweets: any[]): Promise<void> {
  try {
    ...
    const key = `asynch-collection-Sports`;  // Redis key for Sports
...
serve(async (req) => {
  ...
  try {
    console.log("Starting asynchronous sports tweet collection process");
    ...
    console.error("Error in asynch-collection-Sports:", error);
    return new Response(
      JSON.stringify({ 
