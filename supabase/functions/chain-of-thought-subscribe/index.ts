  ...
  try {
    ...
    if (!resendResponse.ok) {
      ...
      if (resendResponse.status === 409 || errorData.message?.includes("Contact already exists")) {
        return new Response(
          JSON.stringify({ error: "You're already subscribed to Chain of Thought!" }),
          {
            status: 409,
            headers: { "Content-Type": "application/json", ...corsHeaders },
          }
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: "Successfully subscribed to Chain of Thought!",
    ...
    console.error("Error in chain-of-thought-subscribe function:", error);
