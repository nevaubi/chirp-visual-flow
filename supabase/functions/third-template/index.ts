...
${formattedTweets}`;
    ...
    try {
      ...
      const markdownSystemPrompt = `You are a creative newsletter designer for "Creative Colorful" by LetterNest. Format pre-analyzed creative content into vibrant, visually engaging, well-structured Markdown with dynamic elements.

NEWSLETTER STRUCTURE:
1.  HEADER: Title (H1 "Creative Colorful"), Date (H3/Subtitle), Creative divider
2.  INTRODUCTION: The "CREATIVE HOOK" with energy
3.  MAIN STORY SECTIONS: For each theme:
    *   Section Title (H2) - creative and engaging
    *   "The Spark" (emphasized/blockquoted introduction)
    *   Image: Include 'RepresentativeImageURL' after "The Spark"
    *   "Key Highlights" (dynamic bulleted list)
    *   "Creative Deep Dive": Present detailed content with:
        *   Creative callout boxes (use distinct formatting for "SPOTLIGHT:", "GAME CHANGER:", "WHY IT MATTERS:")
        *   Visual quotes (format "QUOTE SPOTLIGHT:" distinctly)
        *   Trend alerts (format "TREND ALERT:" with emphasis)
    *   Creative section break
4.  CREATIVE SIDEBARS: Section Title (H2). For each sidebar:
    *   Sub-section Title (H3) - punchy and creative
    *   Image: Include 'RepresentativeImageURL'
    *   "Quick Insight" content
5.  FOOTER: Creative divider, "Crafted with creativity by LetterNest"

FORMATTING GUIDELINES:
- Dynamic Markdown with creative spacing and visual appeal
- Image Usage: Include 'RepresentativeImageURL' prominently (aim for 4-6 images)
- Creative Structures: Format callouts, quotes, and alerts distinctively
- Engaging tone throughout
- Use bold, italic, and creative formatting for visual appeal

OUTPUT: ONLY the formatted Creative Colorful Markdown content.`;
...
You are a creative newsletter UI/UX specialist. Transform "Creative Colorful" markdown into a **visually stunning** document using refined colors with reduced intensity and better navy blues.

REFINED CREATIVE COLOR PALETTE & STYLING:
- **Deep Navy (Primary Headers & Key Text):** #1a365d (Rich, sophisticated navy for H1, H2, primary emphasis)
- **Refined Blue (Secondary Elements):** #2d5a87 (Deeper blue for H3, links, subtle accents)
- **Soft Coral (Warm Accents):** #e57373 (Muted coral for highlights, reduced intensity)
- **Sage Green (Success & Nature):** #81a084 (Softened green for callouts, success elements)
- **Warm Taupe (Neutral Base):** #8d7053 (Earthy brown for variety, grounding)
- **Soft Lavender (Creative Touch):** #9575cd (Muted purple for creative elements)
- **Body Text:** #2d3748 (Rich dark gray for excellent readability)
- **Font:** "Georgia, Times, serif" combined with "Helvetica, Arial, sans-serif" (Creative mix)

ENHANCEMENT RULES:
1.  **Sophisticated Header Hierarchy:**
    *   H1: Deep navy with creative serif styling
    *   H2: Refined blue with medium weight
    *   H3: Sage green with clean sans-serif

2.  **Creative Layout & Refined Spacing:**
    *   Generous margins and elegant spacing
    *   "The Spark" sections: Soft background with refined border
    *   Creative callout variety using different muted colors

3.  **Enhanced Creative Structures:**
    *   **SPOTLIGHT**: Use soft coral background with deep navy text
    *   **GAME CHANGER**: Use sage green background with refined styling  
    *   **WHY IT MATTERS**: Use soft lavender background with navy text
    *   **QUOTE SPOTLIGHT**: Center-aligned with warm taupe accents
    *   **TREND ALERT**: Dynamic styling with refined blue borders

4.  **Refined Visual Elements:**
    *   Elegant dividers using gradient combinations of refined colors
    *   Sophisticated bullet styling with varied muted colors
    *   Images with subtle borders and refined shadows
    *   Creative section breaks with artistic flair

5.  **Email-Client Compatibility:** All inline CSS, refined color choices, elegant presentation

Transform the markdown below into a visually stunning, creatively enhanced newsletter with refined colors.
`;
      
      const enhancedUserPrompt = `
Transform the "Creative Colorful" markdown into a **refined and elegant** version using the sophisticated color palette. Focus on:
- Deep navy (#1a365d) instead of light blues
- Reduced color intensity across all elements  
- Better section color coding with refined palette
- Sophisticated visual hierarchy
- Creative yet elegant styling

Creative Colorful Markdown Draft:
<current newsletter>
${markdownNewsletter}
</current newsletter>
`;
    ...
    // 13) Convert final Markdown to HTML & inline CSS (REFINED CREATIVE STYLING)
    const renderer = new marked.Renderer();
    
    // Override paragraph rendering with creative styling
    renderer.paragraph = (text) => {
        if (text.trim().startsWith('<div style="height: 2px;') || text.trim().startsWith('<div style="background-color:')) {
            return text.trim() + '\n';
        }
        return `<p style="margin: 0 0 1.2em 0; line-height: 1.7; font-size: 16px; color: #2d3748; font-family: 'Helvetica', Arial, sans-serif;">${text}</p>\n`;
    };

    // Creative list item rendering with refined colors
    renderer.listitem = (text, task, checked) => {
      if (task) {
        return `<li class="task-list-item"><input type="checkbox" ${checked ? 'checked' : ''} disabled> ${text}</li>\n`;
      }
      return `<li style="margin: 0 0 0.8em 0; font-size: 16px; line-height: 1.6; color: #2d5a87; font-family: 'Helvetica', Arial, sans-serif;"><span style="color: #2d3748;">${text}</span></li>\n`;
    };

    // Refined heading renderer with sophisticated typography
    renderer.heading = (text, level) => {
      const colors = {
        1: '#1a365d',  // Deep navy
        2: '#2d5a87',  // Refined blue  
        3: '#81a084'   // Sage green
      };
      const sizes = {
        1: '30px',
      ...
      const fonts = {
        1: '"Georgia", Times, serif',
        2: '"Helvetica", Arial, sans-serif',
        3: '"Helvetica", Arial, sans-serif'
      };
      
      const color = colors[level as keyof typeof colors] || '#1a365d';
      const size = sizes[level as keyof typeof sizes] || '18px';
      const font = fonts[level as keyof typeof fonts] || '"Helvetica", Arial, sans-serif';
      
      return `<h${level} style="color:${color};
                               font-size:${size};
                               margin:1.4em 0 0.7em;
                               font-weight:600;
                               font-family: ${font};">${text}</h${level}>\n`;
    };

    // Enhanced image renderer with creative container styling
      ...
      <div class="image-container" style="text-align:center; margin: 20px 0 30px; padding: 0;">
        <img src="${href}"
             ...
             style="max-width:100%; width:auto; max-height:450px; height:auto; border-radius:10px;
                    display:inline-block; box-shadow: 0 4px 16px rgba(26,54,93,0.12); border: 1px solid #e2e8f0;">
      </div>`;

    const htmlBody = marked(finalMarkdown, { renderer });

    // Generate the final email HTML with refined creative design
    const emailHtml = juice(`
      <body style="background-color:#f7fafc; margin:0; padding:0; -webkit-text-size-adjust:100%; font-family: 'Helvetica', Arial, sans-serif;">
        <style>
          @media print {
            body, html { 
              width: 100%; 
          ...
          @media screen and (max-width: 600px) {
            ...
            h1 { font-size: 26px !important; margin: 0 0 16px 0 !important; }
            h2 { font-size: 20px !important; margin: 20px 0 12px 0 !important; }
            h3 { font-size: 18px !important; margin: 16px 0 10px 0 !important; }
            p { font-size: 16px !important; line-height: 1.6 !important; }
            .image-container img { border-radius: 6px !important; max-width: 100% !important; }
          }
          
          @media screen and (min-width: 601px) and (max-width: 900px) {
            .content-container { max-width: 95% !important; }
            .content-body { padding: 30px 25px !important; }
          }
        </style>

        <div class="content-wrapper" style="width: 100%; max-width: 100%; margin: 0 auto; text-align: center; background-color: #f7fafc; padding: 15px 0;">
         <div class="content-container" style="display: block; width: 100%; max-width: 650px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; box-shadow: 0 6px 25px rgba(26,54,93,0.1); text-align: left; border: 1px solid #e2e8f0;">

            <div class="content-body" style="padding: 20px 14px; line-height: 1.7; color: #2d3748; font-size: 16px; font-family: 'Helvetica', Arial, sans-serif;">

              ${htmlBody}
            </div>
          </div>
          
          <div class="footer" style="text-align: center; padding: 30px 0 40px 0; font-size: 14px; color: #81a084; font-family: 'Helvetica', Arial, sans-serif;">
    ...
    logStep("Converted Creative Colorful markdown to HTML with refined design");
