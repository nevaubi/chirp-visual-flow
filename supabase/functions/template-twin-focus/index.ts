...
Provide complete, integrated analysis.`;
      ...
      // 12.5) Generate a catchy title based on the content
      logStep("Generating catchy newsletter title");
      let newsletterTitle = "Twin Focus"; // Default fallback title
      try {
        const titlePrompt = `Generate a catchy, engaging, and concise title (maximum 5 words) for a newsletter based on the content analysis below. The title should be memorable, specific to the content, and capture reader interest. Do not include quotation marks in your response - return ONLY the title text.

CONTENT ANALYSIS:
${finalAnalysisForMarkdown.substring(0, 2000)}`;  // Just use the first portion for title generation
        ...
        const titleRes = await fetch("https://api.openai.com/v1/chat/completions", {
          ...
          body: JSON.stringify({
            model: "gpt-4.1-2025-04-14",
            messages: [
              { role: "system", content: "You are a headline writer who creates catchy, concise titles." },
        ...
        if (titleRes.ok) {
          ...
          // Remove any quotes that might have been included despite instructions
          newsletterTitle = generatedTitle.replace(/['"]/g, '');
          logStep("Successfully generated newsletter title", { title: newsletterTitle });
        } else {
          console.error(`Title generation error (${titleRes.status}):`, await titleRes.text());
          logStep("Failed to generate title, using default");
        }
      } catch (titleErr) {
        console.error("Error generating title:", titleErr);
        logStep("Error in title generation, using default");
      }
...
1. HEADER: Title (H1 with the provided newsletter title), Date (H3/Subtitle), Horizontal rule
...
4. QUICK INSIGHTS: Section Title (H2). For each insight:
   ...
   * Image: If 'RepresentativeImageURL' is valid, include it
   * Summary content
5. FOOTER: Horizontal rule (but DO NOT add any "Generated by" text)
    ...
    @media print{
      ...
      h1,h2,h3{ page-break-after:avoid; }
          ...
          <div style="text-align:center;padding:30px 0 40px 0;font-size:14px;color:#95a5a6;font-family:'Helvetica Neue',Arial,sans-serif;">
            Powered by <span style="font-weight:bold;color:#000000;">letter<span style="color:#FF6B35;">nest</span></span><br>
