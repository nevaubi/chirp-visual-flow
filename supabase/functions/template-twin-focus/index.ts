  ...
  try {
    ...
    function parseToOpenAI(data: any) {
      ...
      arr.forEach((t, i) => {
        ...
        out += `ID: ${t.id}\n`;
        out += `Text: ${txt}\n`;
        out += `Replies: ${t.replyCount || 0}\n`;
        out += `Likes: ${t.likeCount || 0}\n`;
        out += `Impressions: ${t.viewCount || 0}\n`;
        out += `Date: ${dateStr}\n`;
        out += `Author: ${t.author?.name || "Unknown"}\n`;
...
${finalAnalysisForMarkdown}`;
    ...
    renderer.listitem = (text, task, checked) => {
      ...
      return `<li style="margin: 0 0 1em 0; font-size: 16px; line-height: 1.6; color: #333333; font-family: Arial, sans-serif;">${text}</li>\n`;
    };
    ...
    renderer.table = (header, body) => `
      <table style="width:100%;border-collapse:collapse;margin:18px 0;">
        <thead>
          ${header.replace(
      /<th>/g,
      '<th style="background:#f0f2f5;color:#333;padding:8px;font-size:15px;text-align:left;">',
    )}
        </thead>
        <tbody>
          ${body.replace(
      /<td>/g,
      '<td style="padding:8px 10px;font-size:15px;vertical-align:top;">',
    )}
        </tbody>
      </table>`;
      ...
      <body style="background-color:#f8f9fa;margin:0;padding:0;font-family:Arial,sans-serif;">
        <style>
          ...
          @media screen and (min-width:640px){
            .content-body{padding:24px 28px !important;}
          }
        ...
        <div class="wrapper" style="display:block;width:100%;max-width:700px;margin:0 auto;background:#ffffff;border-radius:12px;box-shadow:0 6px 24px rgba(0,0,0,0.1);text-align:left;">
          <div class="content-body" style="padding:20px 14px;line-height:1.7;color:#333;font-size:16px;font-family:Arial,sans-serif;">
            ${htmlBody}
          </div>
        </div>
