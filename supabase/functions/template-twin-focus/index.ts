...
Provide complete, integrated analysis.`;
    ...
    try {
      ...
      const markdownSystemPrompt = `You are a professional newsletter editor for the "Twin Focus" template. Format pre-analyzed content into a visually stunning, layout-rich newsletter with enhanced visual elements, two-column sections, colored blocks, tables, and sophisticated spacing.

NEWSLETTER STRUCTURE:
1. HEADER: Title (H1 "Twin Focus"), Date (H3/Subtitle), Decorative divider
2. INTRODUCTION: The "HOOK" in a highlighted callout box
3. FEATURED TWO-COLUMN SECTION (for the first main focus):
   * Section Title (H2) with colored background indicator
   * Two-column layout with special markers:
     - LEFT COLUMN: {{LEFT_START}} 
       - Featured image (smaller, well-sized)
       - 2-3 key bullet points below image
       - Brief descriptive text
     - {{LEFT_END}}
     - RIGHT COLUMN: {{RIGHT_START}}
       - Multiple shorter paragraphs (2-3 sentences each)
       - Varied formatting with emphasis
       - Key insights highlighted
     - {{RIGHT_END}}
   * Synthesis section below with background color
4. ADDITIONAL FOCUS SECTIONS: For remaining focus areas:
   * Section Title (H2) with colored indicator
   * Image if available
   * Content in structured blocks with background colors
   * Tables for comparative data where appropriate
   * Bullet points and numbered lists for clarity
   * Dividers between sections
5. QUICK INSIGHTS TABLE: 
   * Structured as a comparison table
   * Images integrated into table cells where possible
   * Different row colors for visual appeal
6. FOOTER: Stylized divider, "Generated by LetterNest."

SPECIAL FORMATTING INSTRUCTIONS:
- Use {{SECTION_COLOR_1}}, {{SECTION_COLOR_2}}, etc. to indicate different colored sections
- Use {{DIVIDER}} for visual separators
- Use {{TABLE_START}} and {{TABLE_END}} for structured data
- Use {{CALLOUT_START}} and {{CALLOUT_END}} for highlighted information
- Include generous spacing with {{SPACER}} markers
- Bold and italic formatting for emphasis throughout
- Images should be appropriately sized and positioned

VISUAL ENHANCEMENT REQUIREMENTS:
- Create visually distinct sections with color coding
- Use tables for comparative information
- Implement proper spacing and dividers
- Ensure mobile-friendly responsive design
- Balance text density with white space

OUTPUT: ONLY the formatted Markdown content with special layout markers.`;
      
      const markdownUserPrompt = `Format this Twin Focus analysis into the "Twin Focus" Markdown newsletter with enhanced visual layouts. Use the special formatting markers for visual appeal:

REQUIRED LAYOUT ELEMENTS:
1. Use {{CALLOUT_START}} ... {{CALLOUT_END}} for the hook
2. For the FIRST main focus area, create a two-column featured section:
   - {{LEFT_START}}
   - Include the main image with "featured" in alt text for smaller sizing
   - Add 2-3 key bullet points below the image
   - Brief descriptive paragraph
   - {{LEFT_END}}
   - {{RIGHT_START}}
   - Break the content into 3-4 shorter paragraphs (2-3 sentences each)
   - Use **bold** and *italic* for emphasis
   - {{RIGHT_END}}
3. Use {{SECTION_COLOR_1}}, {{SECTION_COLOR_2}}, {{SECTION_COLOR_3}} for different colored background sections
4. Use {{DIVIDER}} between major sections
5. Use {{SPACER}} for additional spacing
6. Create {{TABLE_START}} comparison tables {{TABLE_END}} for Quick Insights
7. Include {{CALLOUT_START}} boxes for important information

VISUAL STRUCTURE:
- Title and date with decorative elements
- Hook in highlighted callout
- Featured two-column section for first focus
- Colored sections for additional content
- Comparison tables for insights
- Visual dividers and spacers throughout
...
${finalAnalysisForMarkdown}`;
    ...
    // 15) Convert final Markdown to HTML with enhanced Twin Focus styling
    const renderer = new marked.Renderer();
    
    // Enhanced paragraph rendering with layout marker support
    renderer.paragraph = (text) => {
        // Handle special layout markers
        if (text.includes('{{LEFT_START}}')) {
            return `<div class="two-column-container"><div class="left-column">`;
        }
        if (text.includes('{{LEFT_END}}')) {
            return `</div>`;
        }
        if (text.includes('{{RIGHT_START}}')) {
            return `<div class="right-column">`;
        }
        if (text.includes('{{RIGHT_END}}')) {
            return `</div></div>`;
        }
        if (text.includes('{{DIVIDER}}')) {
            return `<div class="visual-divider"></div>`;
        }
        if (text.includes('{{SPACER}}')) {
            return `<div class="spacer"></div>`;
        }
        if (text.includes('{{SECTION_COLOR_1}}')) {
            return `<div class="colored-section section-blue">`;
        }
        if (text.includes('{{SECTION_COLOR_2}}')) {
            return `<div class="colored-section section-green">`;
        }
        if (text.includes('{{SECTION_COLOR_3}}')) {
            return `<div class="colored-section section-purple">`;
        }
        if (text.includes('{{CALLOUT_START}}')) {
            return `<div class="callout-box">`;
        }
        if (text.includes('{{CALLOUT_END}}')) {
            return `</div>`;
        }
        if (text.includes('{{TABLE_START}}')) {
            return `<div class="table-container">`;
        }
        if (text.includes('{{TABLE_END}}')) {
            return `</div>`;
        }
        ...
        return `<p style="margin: 0 0 1.2em 0; line-height: 1.7; font-size: 16px; color: #333333; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">${text}</p>\n`;
    };

    // Enhanced list item rendering with better styling
    renderer.listitem = (text, task, checked) => {
      if (task) {
        return `<li class="task-list-item"><input type="checkbox" ${checked ? 'checked' : ''} disabled> ${text}</li>\n`;
      }
      return `<li style="margin: 0 0 0.8em 0; font-size: 16px; line-height: 1.6; color: #333333; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding-left: 0.5em; position: relative;">
        <span style="color: #4f46e5; font-weight: bold; margin-right: 0.5em;">•</span>${text}
      </li>\n`;
    };

    // Enhanced heading renderer with colored indicators
    renderer.heading = (text, level) => {
      const styles = {
        1: { size: '32px', margin: '0 0 20px 0', color: '#1e293b', weight: '700' },
        2: { size: '26px', margin: '32px 0 16px 0', color: '#334155', weight: '600' },
        3: { size: '22px', margin: '24px 0 12px 0', color: '#475569', weight: '500' },
        4: { size: '18px', margin: '20px 0 10px 0', color: '#64748b', weight: '500' }
      };
      
      const style = styles[level as keyof typeof styles] || styles[4];
      
      // Add colored indicator for H2 headings
      const indicator = level === 2 ? 
        `<span style="display: inline-block; width: 4px; height: 24px; background: linear-gradient(135deg, #4f46e5, #7c3aed); margin-right: 12px; border-radius: 2px; vertical-align: middle;"></span>` : 
        '';
      
      return `<h${level} style="color:${style.color}; font-size:${style.size}; margin:${style.margin}; font-weight:${style.weight}; line-height:1.2; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; display: flex; align-items: center;">
        ${indicator}${text}
      </h${level}>\n`;
    };

    // Enhanced image renderer with responsive sizing
    renderer.image = (href, _title, alt) => {
      // Check if this is in a left column (smaller sizing)
      const isSmallImage = alt && alt.toLowerCase().includes('featured');
      const maxWidth = isSmallImage ? '280px' : '100%';
      const maxHeight = isSmallImage ? '200px' : '400px';
      
      return `
        <div class="image-container" style="text-align:center; margin: 16px 0 20px 0;">
          <img src="${href}"
               alt="${alt || 'Newsletter image'}"
               style="max-width:${maxWidth}; width:auto; max-height:${maxHeight}; height:auto; border-radius:12px;
                      display:inline-block; box-shadow: 0 8px 24px rgba(30,41,59,0.12); border: 1px solid #e2e8f0;
    ...
    renderer.table = (header, body) => {
      return `<div class="table-wrapper" style="margin: 24px 0; overflow-x: auto; border-radius: 12px; box-shadow: 0 4px 16px rgba(30,41,59,0.08);">
        <table style="width: 100%; border-collapse: collapse; background: white; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
          ${header}${body}
        </table>
      </div>`;
    };
    ...
    renderer.tablecell = (content, flags) => {
      ...
      const bgColor = isHeader ? '#f8fafc' : 'white';
      const fontWeight = isHeader ? '600' : '400';
      const color = isHeader ? '#334155' : '#475569';
      
      return `<${tag} style="padding: 16px 20px; text-align: left; background-color: ${bgColor}; font-weight: ${fontWeight}; color: ${color}; font-size: 15px; line-height: 1.5;">
        ${content}
      </${tag}>`;
    };
    ...
    // Enhanced email HTML with comprehensive styling
    const emailHtml = juice(`
      <body style="background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); margin:0; padding:0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
        <style>
        ...
        <div style="width: 100%; max-width: 100%; margin: 0 auto; text-align: center; background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); padding: 20px 0;">
          <div class="content-container" style="display: block; width: 100%; max-width: 700px; margin: 0 auto; background-color: #ffffff; border-radius: 20px; box-shadow: 0 16px 64px rgba(30,41,59,0.15); text-align: left; border: 1px solid #e2e8f0;">
            <div class="content-body" style="padding: 32px 28px; line-height: 1.7; color: #333333; font-size: 16px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
              ${htmlBody}
            </div>
          </div>
          
          <div style="text-align: center; padding: 40px 0 50px 0; font-size: 14px; color: #64748b; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
            <div style="background: white; display: inline-block; padding: 16px 24px; border-radius: 12px; box-shadow: 0 4px 16px rgba(30,41,59,0.08);">
              Powered by <strong style="color: #4f46e5;">LetterNest</strong><br>
              <span style="color: #94a3b8; font-size: 12px;">Professional Newsletter Generation</span>
            </div>
          </div>
        </div>
      </body>
