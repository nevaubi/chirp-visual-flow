...
interface TemplateNewsletterDialogProps {
  open: boolean;
  ...
  <Card 
    className={cn(
      ...
      !disabled && selected 
        ...
        : "border-gray-200 hover:border-[#0078d7]/50 hover:bg-[#0078d7]/5",
      disabled && "hover:border-gray-200 hover:bg-transparent"
    )}
...
const TemplateNewsletterDialog: React.FC<TemplateNewsletterDialogProps> = ({
  open,
  onOpenChange,
  templateId,
  templateName,
  ...
  const [selectedCount, setSelectedCount] = useState<number>(allowedCounts[0]);
  ...
  useEffect(() => {
    if (!open) {
      ...
      setSelectedCount(allowedCounts[0]); // Reset to first allowed count
    }
  }, [open, allowedCounts]);
  ...
  const handleGenerate = async () => {
    try {
      ...
      if (error) {
        ...
        toast.error(`Failed to generate ${templateDisplayName} newsletter`, {
      ...
      setTimeout(() => {
        toast.success(`${templateDisplayName} Newsletter Generated!`, {
          description: `Your ${templateDisplayName.toLowerCase()} newsletter with ${selectedCount} tweets is being processed and will be available soon.`,
        });
      ...
      console.error(`Error in handleGenerate for ${templateName}:`, error);
      toast.error(`Failed to generate ${templateDisplayName} newsletter`, {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="w-[95vw] max-w-[90vw] sm:max-w-md md:max-w-lg max-h-[85vh] sm:max-h-[80vh]">
          <DialogHeader className="space-y-2 sm:space-y-3">
            <DialogTitle className="text-lg sm:text-xl">{templateDisplayName}</DialogTitle>
            <DialogDescription className="text-sm sm:text-base">
              Generate a {templateDisplayName.toLowerCase()} newsletter from your saved bookmarks. 
                ...
                <div className="flex justify-center items-center h-full">
                  ...
                  <div className={`relative w-32 sm:w-48 h-40 sm:h-64 transition-all duration-500 ${progress >= 100 ? 'animate-pulse-soft' : ''}`}>
                    ...
                    <div className={`absolute top-0 left-0 w-32 sm:w-48 h-40 sm:h-64 bg-white border-2 ${progress >= 100 ? 'border-green-400' : 'border-gray-300'} rounded-md shadow-lg overflow-hidden flex flex-col transition-colors duration-300`}>
                      ...
                      <div className={`h-6 sm:h-10 w-full ${progress >= 100 ? 'bg-gradient-to-r from-green-50 to-green-100' : 'bg-gradient-to-r from-[#0078d7]/10 to-[#0078d7]/20'} border-b ${progress >= 100 ? 'border-green-200' : 'border-[#0078d7]/30'} flex items-center justify-center transition-colors duration-300`}>
                        <div className={`w-16 sm:w-24 h-2 sm:h-3 ${progress >= 100 ? 'bg-green-400' : 'bg-[#0078d7]'} rounded-full relative overflow-hidden transition-colors duration-300`}>
                      ...
                      <div className="flex-1 p-2 sm:p-3">
                        {/* Content lines that appear based on progress */}
                        ...
                        <div className={`h-1.5 sm:h-2.5 bg-gradient-to-r ${progress >= 100 ? 'from-green-300 to-green-200' : 'from-[#0078d7]/60 to-[#0078d7]/40'} rounded-full mb-2 sm:mb-3 transition-all duration-700 ease-out delay-100 transform ${progress > 20 ? 'w-5/6 opacity-100' : 'w-0 opacity-0'}`}></div>
                        ...
                        <div className={`h-1.5 sm:h-2.5 bg-gradient-to-r ${progress >= 100 ? 'from-green-300 to-green-200' : 'from-[#0078d7]/60 to-[#0078d7]/40'} rounded-full mb-2 sm:mb-3 transition-all duration-700 ease-out delay-300 transform ${progress > 40 ? 'w-4/5 opacity-100' : 'w-0 opacity-0'}`}></div>
                    ...
                    {progress >= 100 && (
                      <div className="absolute inset-0">
                        ...
                        {Array.from({ length: 20 }).map((_, i) => {
                          ...
                          const colors = [
                            'bg-[#0078d7]', 'bg-[#0078d7]/80',
                            'bg-blue-400', 'bg-blue-500', 'bg-green-400'
                          ];
                  ...
                  {progress < 100 && (
                    <div className="absolute w-full h-full">
                      ...
                      {Array.from({ length: 4 }).map((_, i) => {
                        ...
                        // Template-specific colors
                        const colors = [
                          'text-[#0078d7]/40 opacity-40', 
                          'text-blue-300 opacity-30', 
                          'text-blue-200 opacity-35'
                        ];
                        const colorClass = colors[i % colors.length];
                ...
                <Progress 
                  ...
                  indicatorClassName={cn(
                    ...
                    progress >= 100 
                      ...
                      : "bg-gradient-to-r from-[#0078d7] via-[#0078d7]/80 to-[#0078d7]"
                  )}
                />
              </div>
              ...
              {progress >= 100 ? (
                <div className="text-center space-y-1 sm:space-y-2 px-2">
                  <p className="text-base sm:text-lg font-semibold text-green-700">
                    {templateDisplayName} newsletter generating in background - check Library/email in 2-3 mins!
                  </p>
                ...
                <div className="text-center space-y-1 sm:space-y-2 px-2">
                  <p className="text-xs sm:text-sm text-gray-600">
                    Analyzing your bookmarks and generating {templateDisplayName.toLowerCase()} newsletter...
                  </p>
          ...
          <div className="py-3 sm:py-4 space-y-3 sm:space-y-4">
            <div className="bg-[#0078d7]/10 border border-[#0078d7]/30 rounded-md p-2 sm:p-3 text-xs sm:text-sm text-[#0078d7]">
            ...
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className={cn(
                "grid gap-2 sm:gap-3",
                allowedCounts.length === 2 ? "grid-cols-2" : "grid-cols-3"
              )}>
          ...
          <DialogFooter className="flex flex-col sm:flex-row sm:justify-between items-center gap-3 sm:gap-4 pt-2">
            ...
            <Button 
              type="button" 
              className="bg-[#0078d7] hover:bg-[#0078d7]/90 text-white w-full sm:w-auto order-1 sm:order-2 text-sm sm:text-base py-2 sm:py-3 px-4 sm:px-6"
...
export default TemplateNewsletterDialog;
